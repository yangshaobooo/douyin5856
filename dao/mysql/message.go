...
func QueryLatestChat(userId, friendId int64) (chatContent string, messageType int64, err error) {
	// 查找两个用户的聊天记录
	list, err := MessageChatLatest(userId, friendId)
	if err != nil {
		return "", 0, err
	}
	if len(list) == 0 {
...
func MessageChat(curUserId, toUserID int64, preMegTime time.Time) ([]models.Messages, error) {
	var messageList []models.Messages
	sqlStr := `select id,to_user_id,from_user_id,content,create_time from messages
			where create_time>? and ((to_user_id=? and from_user_id=?) or(to_user_id=? and from_user_id=?))
			order by create_time asc`
	if err := db.Select(&messageList, sqlStr, preMegTime, toUserID, curUserId, curUserId, toUserID); err != nil {
		return nil, err
	}
	// 没有错误
...
func MessageChatLatest(curUserId, toUserID int64) ([]models.Messages, error) {
	var messageList []models.Messages
	sqlStr := `select id,to_user_id,from_user_id,content,create_time from messages
